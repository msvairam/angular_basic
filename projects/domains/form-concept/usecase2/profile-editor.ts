...
@Component({
  ...
  template: `
    ...
    <form [formGroup]="profileForm" (ngSubmit)="submit()">
      ...
      <br />
      Email:
      <p>{{ profileForm.get('userEmail')?.value ?? '' | maskEmail: 1 }}</p>
      ...
      <button (click)="clearAliases()">Clear All</button>
...
export class ProfileEditor {
  ...
  /*
    TypeScript will enforce that you always handle the possibility that the control has become null. 
    If you want to make this control non-nullable, you may use the nonNullable option. 
  ...
  protected addAliases(): void {
    this.aliases.push(this.formBuilder.nonNullable.control('', [Validators.required]));
  }
  ...
  ngAfterViewInit() {
    ...
    this.profileForm.updateValueAndValidity({onlySelf: true, emitEvent: false});
